...
1) Пользователь переходит по рекомендации со страницы каталога
2) API Gateway отправляет событие в очередь о клике пользователя и его контекст
3) Сообщение из очереди приходит в сервис рекомендаций. Он обновляет веса в векторном хранилище
4) При заходе пользователя на страницу каталога, сервис рекомендаций считает top-N товаров и присылает их пользователю

## 6. Выделение модулей и протоколов взаимодействия

### Основные компоненты (модули) и их ответственность:
...
### Модули с протоколами взаимодействия
![protocols](docs/diagrams/architecture_protocols_v1.png)

- Фронт общается с бэкендом через http
- БД общается с бэкендом через http
- Очередь (Кафка) взаимодействует с сервисами по kafka protocol
- Хранилище признаков общается с сервисом рекомендаций по http
- Кеш (редис) по redis protocol

## 7. Выбор технологий и обоснование
 - <b>Bandit engine</b> - Python + NumPy/SciPy. Быстрый прототипинг, богатая экосистема ML в отличие от более производительных rust и java
 - <b>API Gateway</b> - FastAPI, т.к. это современный востребованный асинхронный. В Django загоняет в рамки, нет свободы, flask медленнее, меньше возможностей
 - <b>Feature store</b> - ChromaDB. Проста в использовании. Не зависит от других модулей, например как pgvector
 - <b>Queue</b> - Kafka, т.к. высокая пропускная способность и replay capability - возможность считывать метрики повторно. Может быть полезно при пересчете весов. Из альтернатив RabbitMQ, где этот механизм не так хорошо работает и пропускная способность меньше
 - <b>Database</b> - PostgreSQL. Стандартная SQL БД для ec-commerce продуктов
 - <b>Cache</b> - Redis. Самый распространенный и развитый инструмент для кеширования

## Источники
Датасет ZOZO на GitHub: https://github.com/st-tech/zr-obp  
Страница датасетов ZOZO: https://research.zozo.com/data.html